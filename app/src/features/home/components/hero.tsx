...
export function Hero({
  ...
  return (
    <section className="hero-background overflow-hidden border-b border-slate-200 pt-18 dark:border-slate-800">
      <div className="mx-auto flex w-full max-w-7xl flex-col items-center justify-center px-6 lg:flex-row lg:justify-between lg:gap-14">
        <div className="max-w-148 py-16 text-center lg:py-24 lg:text-left">
          <div className="flex flex-wrap items-center justify-center gap-4 text-sm font-light text-slate-600 lg:justify-start dark:text-slate-400">
            <span>
              {t("{count} icons", {
            ...
            <span>
              {t("{count} icon sets", {
                count: setCount.toLocaleString(language),
              })}
            </span>
            <Dot />
            <span>{t("Open source collections")}</span>
          </div>
          <h1 className="mt-6 text-4xl leading-tight font-semibold tracking-tight text-balance">
            {t("Beautiful SVG icons")}
            <span className="text-[#bfc2bd]">
              {" "}
              {t("ready for your next interface.")}
            </span>
          </h1>
          <p className="mt-6 text-base text-[22px] leading-[1.4] tracking-[-0.3px] text-slate-600 md:tracking-[-0.5px] dark:text-slate-400">
            {t(
          ...
          <div className="mt-10 flex flex-wrap justify-center gap-4 lg:justify-start">
            <ActionLink href={paths.icons} variant="solid">
              {t("Browse icons")}
              <Icon icon="tabler:arrow-up-right" size={18} />
            </ActionLink>
            <ActionLink href={paths.guide}>
              {t("Usage guide")}
              <Icon icon="tabler:arrow-up-right" size={18} />
            </ActionLink>
          </div>
          <a
            href="#ui-examples"
            className="mt-6 inline-flex items-center gap-2 rounded text-sm font-medium text-slate-500 hover:text-primary focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-primary dark:text-slate-400"
          >
            {t("Try the UI examples")}
            <Icon name="tabler:arrow-down" size={16} aria-hidden="true" />
          </a>
        </div>
...
function Dot() {
  return (
    <span
      className="inline-flex size-1 rounded-full bg-current opacity-45"
      aria-hidden="true"
    />
  )
}

function HeroIconBoard() {
  return (
    <div className="hidden lg:flex lg:flex-auto lg:justify-center">
      <div className="relative aspect-square w-72 flex-none">
        ...
        <div className="absolute inset-0 bg-white/1 backdrop-blur-[2px]"></div>
        <svg
          viewBox="0 0 288 288"
          fill="none"
          aria-hidden="true"
