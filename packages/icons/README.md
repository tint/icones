...
支持 JSON import attributes 的环境可以直接导入单个图标：

```ts
import star from "@icones/icons/tabler/data/star.json" with { type: "json" }
```

在 Vite 项目中通常不需要直接导入本包；`@icones/vite` 会按静态使用名称读取所需资源，不会把完整图标库打入客户端 JavaScript。
...
每个集合的授权信息位于 `<set>/license.txt`，来源和版本记录在 `<set>/manifest.json`。直接再分发资源时应同时保留对应许可证；不同图标集的授权条件彼此独立。